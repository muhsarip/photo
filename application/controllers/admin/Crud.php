...
class Crud extends ADMIN_Controller {
    ...
    public function users()
    {
		...
		{
	    	$crud->add_fields('username', 'email', 'phone','groups' , 'password', 'password_confirm');
			$crud->edit_fields('username', 'email', 'phone','groups' , 'last_login','old_password','new_password');
		}else{
		...
		$crud->add_action('User Access', '', 'admin/crud/user_access','ui-icon-plus');
    ...
    public function account()
    {
    	...
    	if (isset($_POST)) {
	    	...
	    	if ($iduser != $this->user->id) {
	    		redirect(base_url("admin/crud/index"),'refresh');
	    	}
    	}
...
, 'email', 'phone','groups' , 'password', 'password_confirm');
			$crud->edit_fields('username', 'email', 'phone','groups' , 'last_login','old_password','new_password');
		}else{
	...
	public function settings()
	{
		...
		$crud->set_field_upload('bg_auth','assets/img');
		$crud->columns('logo','favicon','judul','nama_perusahaan','alamat','skin','bg_auth');
		//$crud->callback_field('bg_auth',array($this,'cb_color'));
		$crud->display_as("skin","Background sidebar");
		$crud->unset_add();
		$crud->unset_delete();
		$crud->unset_export();
		$crud->unset_print();
		$crud->unset_texteditor(array('footer'));
